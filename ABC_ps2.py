...
def generateInteractionMatrix():
    ...
    interaction_matrix = [
                ...
                [-0.52, 0, 0, 0.2, 0, 0, 0, 0, 0, 0.016, 0.4],
                # exmoor pony - special case, no growth
                [0, -0.001, 0, 0, 0, 0, 0, 0, 0, 0, 0],
                # fallow deer 
                [0, 0, -0.08, 0.31, 0, 0, 0, 0, 0, 0.0048, 0.27], #parameter set 2
                # grassland parkland
                [0, -0.00045, -0.00011, -0.76, -0.00013, -0.00024, 0, -0.00026, -0.0016, -0.011, -0.14], #parameter set 2
                # longhorn cattle  
                [0, 0, 0, 0.64, -0.72, 0, 0, 0, 0, 0.032, 0.3], #parameter set 2
                # organic carbon
                [0, 0.00019, 0.00092, 0.13, 0.0006, -0.1, 0.002, 0.0025, 0.00068, 0.0015, 0.025],  
                # red deer  
                [0, 0, 0, 0.47, 0, 0, -0.26, 0, 0, 0.0027, 0.4],
                # roe deer 
                [0, 0, 0, 0.83, 0, 0, 0, -0.97, 0, 0.035, 0.74],
                # tamworth pig 
                [0,  0, 0, 0.24, 0, 0, 0, 0, -0.99, 0.0024, 0.71],  
                # thorny scrub
                [0, -0.027, -0.019, 0, -0.052, 0, -0.019, -0.025, -0.017, -0.012, -0.1],
                # woodland
                [0, -0.0036, -0.003, 0, -0.0047, 0, -0.0033, -0.0027, -0.0039, 0.00065, -0.0092]
    ]

    # generate random uniform numbers
    variation = np.random.uniform(low = 0.96, high=1.04, size = (len(species),len((species))))
...
def generateGrowth():
    ...
    growthRates = [0.058, 0, 0.083, 0.94, 0.1, 0.041, 0.2, 0.14, 0.44, 0.79, 0.046]
    # multiply by a range
    variation = np.random.uniform(low = 0.96, high=1.04, size = (len(species),))
...
# check the final runs
    ...
    with open("ps2_stable_numberStable.txt", "w") as text_file:
...
def filterRuns_2():
    ...
    (final_runs_2['fallowDeer'] <= 4.8) & (final_runs_2['fallowDeer'] >= 3.6) &  # leaning on methods
    (final_runs_2['longhornCattle'] <= 2.7) & (final_runs_2['longhornCattle'] >= 2.3) & # leaning on methods
    (final_runs_2['tamworthPig'] <= 1.7) & (final_runs_2['tamworthPig'] >= 0.6)]
    ...
    (filtered_2015['fallowDeer'] <= 5.1) & (filtered_2015['fallowDeer'] >= 3.9) & # leaning on methods
    (filtered_2015['longhornCattle'] <= 2.4) & (filtered_2015['longhornCattle'] >= 2) & 
    (filtered_2015['tamworthPig'] <= 1.5) & (filtered_2015['tamworthPig'] >= 0.4)]
    ...
    (filtered_2016['longhornCattle'] <= 2.3) & (filtered_2016['longhornCattle'] >= 1.9) & 
    (filtered_2016['redDeer'] <= 2.2) & (filtered_2016['redDeer'] >= 1.8) & # leaning on data
    (filtered_2016['tamworthPig'] <= 2) & (filtered_2016['tamworthPig'] >= 0.9)]
    ...
    (filtered_2017['fallowDeer'] <= 8.6) & (filtered_2017['fallowDeer'] >= 7.4) & 
    (filtered_2017['longhornCattle'] <= 2.5) & (filtered_2017['longhornCattle'] >= 2.1) & 
    (filtered_2017['redDeer'] <= 3.3) & (filtered_2017['redDeer'] >= 2.8) & 
    (filtered_2017['tamworthPig'] <= 1.7) & (filtered_2017['tamworthPig'] >= 0.6)]
    ...
    (filtered_2018['fallowDeer'] <= 8.1) & (filtered_2018['fallowDeer'] >= 7) & 
    (filtered_2018['longhornCattle'] <= 2.4) & (filtered_2018['longhornCattle'] >= 2) & 
    (filtered_2018['redDeer'] <= 3.6) & (filtered_2018['redDeer'] >= 3.2)]
...
def generateParameters3():
    ...
    ax.set_yticklabels(
        ax.get_yticklabels(), 
        fontsize = 5)
    plt.savefig('corrMatrix_ps25.png')
    # plt.show()
...
def runODE_3():
    ...
    for X0_exp2, r_9, A_9 in zip(X0_experiment2,r_thirdRun, np.array_split(A_thirdRun,len(accepted_simulations_2020))):
        ...
        stocking_fallowDeer = 0
        stocking_longhornCattle = 0
        stocking_redDeer = 0
        stocking_tamworthPig = 0
    ...
    # select the runs where grassland declines by >= 90% (0.13)
    accepted_simulations_exp2 = accepted_year_exp2[(accepted_year_exp2['grasslandParkland'] <= 0.19)]
    print("number accepted, experiment 2:", accepted_simulations_exp2.shape)

    with open("ps2_stable_numberAcceptedExp2.txt", "w") as text_file:
    ...
    for X0_exp25, r_9, A_9 in zip(X0_experiment2,r_thirdRun, np.array_split(A_thirdRun,len(accepted_simulations_2020))):
        # take 10% above and below the optimizer outputs; parameter set 2
        stocking_exmoorPony_25 =  np.random.uniform(low=3, high=3.2)
        stocking_fallowDeer_25 = np.random.uniform(low=4.4, high=4.6)
        stocking_longhornCattle_25 = np.random.uniform(low=4.3, high=4.5)
        stocking_redDeer_25 = np.random.uniform(low=4.5, high=4.7)
        stocking_tamworthPig_25 = np.random.uniform(low=0.8, high=1)
    ...
    with open("ps2_stable_numberAcceptedExp25.txt", "w") as text_file:
    ...
    for XO_bisonReintro, r_5, A_5 in zip(X0_euroBison,r_thirdRun, np.array_split(A_thirdRun,len(accepted_simulations_2020))):
        ...
        A_5[3][0] = np.random.uniform(low=-0.001, high=-0.00045)
        A_5[5][0] = np.random.uniform(low=0.0025, high=0.01)
        A_5[9][0] = np.random.uniform(low=-0.1, high=-0.052)
        A_5[10][0] = np.random.uniform(low=-0.01, high=-0.0047)
...
def plotting():
    ...
    plt.savefig('acceptedRejected_ps25.png')
    # plt.show()
    ...
    plt.tight_layout()
    plt.savefig('zeroGrass_ps25.png')
    # plt.show()
