...
def run_model(X0, A, r, exmoor_stocking, fallow_stocking, longhorn_stocking, red_stocking, tamworth_stocking):
    ...
    for year in range(years): 
        ...
        starting_values[3] = 1.5 + (1.5*longhorn_stocking)
        starting_values[4] = 2.7 + (2.7*red_stocking)
        starting_values[6] = 0.55 + (0.55*tamworth_stocking)
...
def objectiveFunction(x):
    ...
    exmoor_stocking = x[0]
    fallow_stocking = x[1]
    longhorn_stocking = x[2]
    red_stocking = x[3]
    tamworth_stocking = x[4]
...
def graph_results():
    ...
    longhorn_stocking = output_parameters["variable"][2]
